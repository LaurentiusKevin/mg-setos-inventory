            ...
            <div class="card">
                <div class="card-header">
                    <div class="card-header-actions">
                        <div class="card-header-actions">
                            <a class="btn btn-success btn-block btn-sm" href="{{ route('admin.system-utility.menu.view.create') }}">Tambah</a>
                        </div>
                    </div>
...
<script type="text/javascript">
    const t_list = $('#t_list').DataTable({
        ...
        scrollX: true,
        ajax: {
            url: '{{ route('admin.system-utility.menu.api.data') }}',
            method: 'post'
        },
        columns: [
            {data: 'ord', width: '5%'},
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        t_list_tbody.on('click','button.action-edit', function (event) {
            ...
            window.location = `{{ url('admin/system-utility/menu/edit') }}/${data.id}`;
        });
        ...
        t_list_tbody.on('click','button.action-change-status', event => {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios({
                        url: '{{ route('admin.system-utility.menu.api.show-unshow') }}',
                        method: 'post',
                        data: {
                            id: data.id
                        }
        ...
        t_list_tbody.on('click','button.action-delete', event => {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios({
                        url: '{{ route('admin.system-utility.menu.api.delete') }}',
                        method: 'post',
                        data: {
                            id: data.id
                        }
